                        ...
                        <table class="table table-hover">
                            <tbody>
                                ...
                                <tr v-for="marca in marcas.data" :key="marca.id ">
                                    <td>{{marca.id}}</td>
                                    <td>{{marca.marca}}</td>
                                    <td><img :src="marca.file" style="width:120px;"></td>

                                    <td>
                                        <a @click="editMarca(marca)">
                        ...
                        <div class="modal-body">
                            <div class="form-group col-12">
                                ...
                                <input
                                    ...
                                    class="form-control cap" :class="{ 'is-invalid': form.errors.has('marca') }">
...
export default {
    ...
    methods:{
        deleteMarca(id){
            swal({
                ...
                this.form.delete('api/marcas/'+id).then(()=>{
                        if (result.value) {
                        swal(
                            'Eliminado!',
                            'success'
        ...
        getResults(page = 1) {
            axios.get('api/marcas?page=' + page)
                .then(response => {
        ...
        updateMarca(){    
            ...
            this.form.put('api/marcas/'+this.form.id)
            .then(()=>{
                $('#marca').modal('hide');
                this.loadMarcas();
                swal(
        ...
        createMarca(){            
            this.form.file = this.image;
            this.form.post('api/marcas').then(()=>{
                $('#marca').modal('hide');
                this.loadMarcas();
                toast({
                    type: 'success',
    ...
    created() {
        Fire.$on('searching',()=>{
            ...
            axios.get('api/findMarca?q='+query)
            .then((data)=>{
